(function( $ ) {
	...
	function acadp_osm_render_map( $el ) {
		...
		$ ( '.acadp-map-field', '#acadp-contact-details' ).on( 'blur', function() {
			...
			if ( $location.val() > 0 ) {   
				query.push( $location.text().trim() );

				if ( ! $location.hasClass( 'level-0' ) ) {
					var $prev = $location.prev();

					while ( ! $prev.hasClass( 'level-0' ) ) {						
						$prev = $prev.prev();					
					}

					query.push( $prev.text() );
				}				
			}
	...
	$(function() {
		...
		// Common: Initialize the datetime picker	
		...
		$( '.field-type select', '#acadp-field-details' ).on( 'change', function() {			
			var option = $( this ).val();
			var num_fields = $( '.field-options', '#acadp-field-details' ).length;
			...
			$( '.field-options', '#acadp-field-options' ).fadeOut( 200, function() {
				$( '.field-option-' + option, '#acadp-field-options' ).fadeIn( 200 );
			});	
		}).change();
		...
		$( '#acadp_category' ).on( 'change', function() {
			...
			var cached = $( '#acadp-custom-fields-list' ).attr( 'data-cache' );
			...
			$( '#acadp-custom-fields-list' ).attr( 'data-cache', JSON.stringify( fields ) );
			...
			$.post( ajaxurl, data, function( response ) {
				...
				$( '.acadp-date-picker, .acadp-datetime-picker', '#acadp-custom-fields-list' ).each(function() {
					acadp_initialize_datetime_picker( $( this ) );
				});

				$( '.acadp-range-slider', '#acadp-custom-fields-list' ).each(function() {
					acadp_initialize_range_slider( $( this ) );
				});
			});			
		});		
